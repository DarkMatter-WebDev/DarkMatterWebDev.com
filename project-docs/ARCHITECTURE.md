...
- `assets/surette-data-systems-logo.webp`, `assets/surette-data-systems-geometric-icon.webp`, and `assets/surette-data-systems-floating-orange-blocks.webp`: lossless WebP app-brand assets.
- `assets/case-studies-building-blocks.webp`: lossless WebP floating block visual used in the case studies hero.
...
- `assets/client-portal.js` also renders the super-admin account-holder table by calling a Supabase RPC instead of reading `auth.users` directly from browser code.
...
- `assets/supabase-config.js` stores public Supabase config plus non-secret email allowlists for browser-side UI fallback. It points to the dedicated Dark Matter / Surette Data Systems Supabase project, not the Naples Estate Jewelry project.

## Auth/Portal

- Portal uses Supabase browser auth for email/password and magic-link style flows.
- Portal auth/data lives in a dedicated Dark Matter / Surette Data Systems Supabase project fully separated from Naples Estate Jewelry.
...
- SQL helpers in `supabase/portal-role-setup.sql` support RLS policies and privileged browser-safe admin reads. `list_portal_account_holders()` is a security-definer RPC that returns auth/profile account rows only when the caller is a super admin.
...
- Contact, request, and checkout flows use static/Netlify form markup.
