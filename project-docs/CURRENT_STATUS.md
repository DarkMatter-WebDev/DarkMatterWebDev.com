...
- Desktop service pages share the same Surette Data Systems footer block with legal/resource links and `info@SuretteSystems.com`.
...
- Global page reveal still uses `html` opacity controlled by `surette-logo.js`, with a sitewide no-white-flash baseline: critical inline head CSS on HTML pages, black `html`/`body`/shell backgrounds in `assets/nav.css`, dark route/app loader surfaces, and `theme-color` set to `#050505`. Same-site link clicks briefly show the small black spinner overlay before navigation to avoid a frozen-page feel. Repeated cards/panels/tiles use a shared object-level reveal so their text and background treatment appear together after page-ready.
