...
- Global page reveal still uses `html` opacity controlled by `surette-logo.js`, with black `html`/`body` backgrounds and a CSS safety fallback so logo pages cannot stay hidden if the JS reveal is interrupted. Same-site link clicks briefly show the small black spinner overlay before navigation to avoid a frozen-page feel.
...
- Static validator reports pre-existing noise: missing `es/` mirrors, `node_modules` HTML scan, stale Sean Ads Spanish links, mojibake warnings, plus the old `blackhole-icon.html` missing-video warning. New English-only portal admin pages also appear as missing Spanish mirrors until bilingual mirrors are restored.
