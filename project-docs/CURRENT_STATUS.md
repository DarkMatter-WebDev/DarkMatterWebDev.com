...
- Global page reveal still uses `html` opacity controlled by `surette-logo.js`, with black `html`/`body` backgrounds and a CSS safety fallback so logo pages cannot stay hidden if the JS reveal is interrupted.
