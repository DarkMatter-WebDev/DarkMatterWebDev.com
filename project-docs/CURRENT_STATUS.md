...
- Horizontally-scrollable tables self-announce: `assets/table-scroll-hint.js` nudges any overflowing table ~52px right and back the first time it scrolls into view (tablet/mobile only, skipped under `prefers-reduced-motion`, cancelled by user interaction). It finds the scroll container by walking up from every `<table>`, so new tables are covered automatically — do not add a per-table class or opt-in. Loaded on `services/website-design-hosting.html`, `account-admin.html`, `account-users.html`.
