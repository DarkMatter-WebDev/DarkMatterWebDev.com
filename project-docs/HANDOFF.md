...
- **Build-cost estimates on the portfolio (2026-07-16).** Every Websites gallery tile in `casestudies.html` (both the desktop and mobile grids — 12 pills total) shows a gold `.portfolio-tile__price` pill at the bottom-right of the tile image ("EST. $12.5K+" style; bottom-right so it can't collide with the top-left number badge on 2-up phone tiles). Every portfolio detail page has a `#build-pricing` "What would a build like this cost?" section before its final CTA: estimate range, closest one-time package from `website_pricing_plan.txt`, itemized scope drivers, CTAs to `services/website-design-hosting.html#one-time-builds` and `contact.html`, and the estimate-only disclosure the pricing spec requires. Estimates: Naples $12K–$16K, Elite $8.5K–$10K, JPSurette $4.5K–$6.5K, SeansAds $7.5K–$10K, AuctionBuddha $35K–$55K+ (repriced 2026-07-16 — see the entry above), MetalsCalc $2K–$2.5K. Note: `portfolio-seansads.html` gets its Tailwind config from `assets/tailwind-nav-config.js`, so its section uses `font-['JetBrains_Mono']`-style arbitrary classes rather than the `font-label-mono` tokens the other detail pages define inline. If pricing changes in `website_pricing_plan.txt`, both surfaces must be updated together.
